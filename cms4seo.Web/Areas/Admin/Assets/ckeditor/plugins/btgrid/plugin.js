          ...
          // Before init.
           ...
           data: function() {
             if (this.data.colCount && this.element.getChildCount() < 1) {
               ...
               var alignSelfCenter = this.data.alignSelfCenter;
               var row = this.parts['btgrid'];
               for (var i= 1;i <= rowCount;i++) {
                 this.createGrid(colCount, row, i, alignSelfCenter);
               }
             }
           },
           //Helper functions.
           // Create grid
           createGrid: function(colCount, row, rowNumber, alignSelfCenter) {
               var dFlex, alignSelf = "";
               if (alignSelfCenter) {
                   dFlex = "d-flex";
                   alignSelf = "align-self-center";
               }
             var content = '<div class="row row-' + rowNumber + ' ' + dFlex + '">';
             for (var i = 1; i <= colCount; i++) {
               content = content + '<div class="col-md-' + maxGridColumns/colCount + ' ' + alignSelf + '">' +
                                   '  <div class="content">' +
